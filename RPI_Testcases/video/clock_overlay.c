...
typedef struct _CustomData {
	GstElement *pipeline,*source,*decoder,*clock,*videoconv,*videosink;
...
static int show_Time(CustomData *data,const char* text,gint flag,guint shade_val,gint horizont,gint vertical,gchar *font,gint resize,guint clr)
{
	...
	autoresize = resize;
	color=clr;
	g_object_set(G_OBJECT(data->clock),"text",text,"shaded-background",shade_background,"shading-value",shade_value,"halignment",hval,"valignment",vval,"font-desc",font,"auto-resize",autoresize,"color",color,NULL);
	gchar *ltext;
	gint lflag;
	guint lshade_val;
	gint lhal,lver;
	...
	gint lautoresize;
	guint lclr;
	g_object_get(data->clock,"text",&ltext,"shaded-background",&lflag,"shading-value",&lshade_val,"halignment",&lhal,"valignment",&lver,"font-desc",&lfont,"auto-resize",&lautoresize,"color",&lclr,NULL);
...
gint mine()
{
	...
	data.clock=gst_element_factory_make("clockoverlay","display time");
	if(!data.clock){
...
//	data.videosink = gst_element_factory_make ("fpsdisplaysink", "video output");
	...
	if (!data.pipeline || !data.source || !data.decoder || !data.clock || !data.videoconv || !data.videosink ) {
	...
	gst_bin_add_many (GST_BIN (data.pipeline),
			data.source,data.decoder,data.clock,data.videoconv,data.videosink,NULL);
	/* we link the elements together */
	gboolean re;
	re = gst_element_link (data.source, data.decoder);
	if(re == FALSE){
		g_print("could not link source ,decoder\n");
		return -1;
	}
	
	re = gst_element_link (data.videoconv,data.clock);
	if(re == FALSE){
		g_print("could not link converter,clock\n");
		return -1;
	}
	re = gst_element_link (data.clock,data.videosink);
	if(re == FALSE){
		g_print("could not link clock ,sink\n");
		return -1;
	}
	...
	data.loop = g_main_loop_new (NULL, FALSE);
	//gchar *s = "ramya";
	p = show_Time(&data,"Now time is",1,250,0,3,"Garuda,20",1,4294967295);
