...
extern unsigned char rec_buff[25],time[20];
...
/** display 4 character string on dot matrix's **/

void display_string(unsigned char *str,unsigned char delay)
{
	unsigned int j=0,l;	//local variables
//based on delay value same string is displyed on dot matrix
//delay is used for user visibility
	for(l=0;l<delay;l++)	
	{
		...
		{
			row=0x00;
			tx(font[str[0]-32][j],1);	//display 1st character
			tx(font[str[1]-32][j],2);	//display 2nd character
			tx(font[str[2]-32][j],3);	//display 3rd character

			tx(font[str[3]-32][j],4);	//display 4th character
...
/** rotate the string (changing the array elements) **/
void rotate(unsigned char *str)
{
	unsigned char x,temp;	//local variable
	temp=str[0];	//saving 0the element to temporary variable
	for(x=0;x<strlen(str)-1;x++)
		str[x]=str[x+1];//shifting the elements to left by one position
	str[x]=temp;	//saving temporary variable value to last position in the array
...
void rotate_time()
{
	unsigned char x;	//local variable
	for(x=0;x<strlen(time)-1;x++)
		time[x]=time[x+1];//shifting the elements to left by one position
	time[x]='\0';	//saving temporary variable value to last position in the array
}
