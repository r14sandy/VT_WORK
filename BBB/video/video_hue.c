...
static int change_hue(CustomData *data,gdouble i)
{
	hue+=i;
	gdouble lhue;
	g_object_set(G_OBJECT(data->vidbalance),"hue",hue,NULL);
	g_object_get(G_OBJECT(data->vidbalance),"hue",&lhue,NULL);
	if(hue==lhue)
		return 1;
	else
		return 0;
