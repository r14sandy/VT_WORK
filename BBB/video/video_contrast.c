...
static int change_contrast(CustomData *data,gdouble i)
{
	cont+=i;
	gdouble lcont;
	g_object_set(G_OBJECT(data->vidbalance),"contrast",cont,NULL);
	g_object_get(G_OBJECT(data->vidbalance),"contrast",&lcont,NULL);
	if(cont==lcont)
		return 1;
	else
		return 0;
