...
gint start()
{
	...
	g_signal_connect (data.videoconv, "pad-added", G_CALLBACK (on_pad_added), data.videobox);
	g_signal_connect(data.videobox, "pad-added", G_CALLBACK(on_pad_added), data.videosink);   
	...
	data.loop = g_main_loop_new (NULL, FALSE);
	gint p;
	p = change_borders(&data,-100,-100,-100,-100,3);
