...
typedef struct _CustomData {
	GstElement *pipeline,*source,*decoder,*videoconv,*audioconv,*audiosink,*videosink,*resample,*cap;
...
static void change_resolution(CustomData *data)
{
	g_object_set(data->cap,"caps-change-mode",)
...
int main (int argc,char *argv[])
{
	...
	data.cap=gst_element_factory_make("capsfilter","capfilt");
	if(!data.cap){
                g_print("Failed to create cap filter\n");
                return -1;
        }
	...
	gst_bin_add_many (GST_BIN (data.pipeline),
			data.source,data.decoder,data.audiosink,data.videosink,data.cap,data.resample,data.audioconv,data.videoconv,NULL);
	/* we link the elements together */
	gst_element_link (data.source, data.decoder);
	gst_element_link (data.decoder,data.videoconv);
	gst_element_link(data.videoconv,data.cap);
	gst_element_link(data.cap,data.videosink);
//	gst_element_link (data.videoconv,data.videosink);
	...
	g_signal_connect(data.videoconv, "pad-added", G_CALLBACK(on_pad_added), data.cap);
	g_signal_connect(data.cap, "pad-added", G_CALLBACK(on_pad_added), data.videosink);      
