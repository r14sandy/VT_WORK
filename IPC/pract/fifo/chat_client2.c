...
{
	char name[10],phone[11],passwrd[11];
...
void signin()
{
	...
	scanf("%s",ph);
	if(!number_verify(ph))
		puts("number is not exist,please create account");
	else
	{
...
//		if(flag==0)
//		puts("phone,password are not matching");
...
void forgotpw()
{
	...
	if(flag==0)
		puts("number not found,please creat the account first");
	fclose(fp);
}
